...
function pintarAdmin() {
  ...
  const pendClaves = (BD.solicitudesClave || []).filter(s => !s.atendida).length;
  $('#gClaves').textContent = pendClaves || '';

  tablaUsuarios();
  tablaArticulos();
  tablaAnuncios();
  listaSolicitudes();
  listaSolicitudesClave();
  listaPagos();
  listaSeguros();
  listaRiesgo();
  pintarReportes();
...
/* Solicitudes de "olvidé mi código de respaldo", pedidas al admin */
function listaSolicitudesClave() {
  const cont = $('#listaClaves');
  if (!cont) return;
  const lista = BD.solicitudesClave || [];
  cont.innerHTML = lista.length ? lista.map(s => {
    const u = s.usuario ? usuario(s.usuario) : null;
    return `<div class="fila">
      <div class="fila-info">
        <b>${esc(s.nombre || (u && u.nombre) || 'Sin cuenta con ese correo')}
          ${s.atendida ? '<span class="eti verde">Atendida</span>' : '<span class="eti oro">Pendiente</span>'}</b>
        <span>✉️ ${esc(s.email)} · 🕓 ${fechaTxt(s.creado)}</span>
        ${s.nota ? `<span>💬 "${esc(s.nota)}"</span>` : ''}
        ${u ? `<span>🔑 Código actual: <b>${esc(u.codigo || '—')}</b></span>`
             : `<span style="color:var(--rojo,#c0392b)">No hay ninguna cuenta con ese correo.</span>`}
      </div>
      <div class="fila-acc">
        ${u ? `<button class="btn-t" data-copiarclave="${esc(u.codigo || '')}">📋 Copiar código</button>
               <a class="btn-t" target="_blank" rel="noopener"
                 href="mailto:${esc(s.email)}?subject=${encodeURIComponent('Tu código de respaldo — Truequea PE')}&body=${encodeURIComponent('Hola ' + (u.nombre || '') + ', tu código de respaldo es: ' + (u.codigo || ''))}">✉️ Enviar por correo</a>` : ''}
        ${!s.atendida ? `<button class="btn-t verde" data-atenderclave="${s.id}">✓ Marcar atendida</button>` : ''}
      </div></div>`;
  }).join('') : `<p class="nota">No hay solicitudes de código de respaldo.</p>`;
}
...
function pintarEstadoNube() {
  const c = $('#estadoNube');
  if (!c) return;
  const n = typeof NUBE !== 'undefined' ? NUBE : null;
  const cfg = typeof SUPABASE_CONFIG !== 'undefined' ? SUPABASE_CONFIG : null;
  c.className = 'estado-nube ' + (n && n.activa && !n.error ? 'ok' : 'no');
  c.innerHTML = `<b>Supabase · ${n && n.activa ? 'lectura disponible' : 'sin conexión confirmada'}</b>
    <span>Proyecto: ${esc(cfg ? cfg.supabaseUrl : 'No configurado')}</span>
    <span>Tabla: ${esc(cfg ? cfg.tabla : '—')}</span>
    <span>Tiempo real: ${n && n.conectado ? 'canal suscrito' : 'sin suscripción confirmada'}</span>
    <span>Última lectura: ${esc(n && n.ultimaLectura ? n.ultimaLectura.toLocaleString() : 'Sin confirmar')}</span>
    <span>Última escritura: ${esc(n && n.ultimaEscritura ? n.ultimaEscritura.toLocaleString() : 'Sin confirmar en esta sesión')}</span>
    <span>${n && n.pendiente ? 'Hay cambios pendientes de envío.' : 'No hay envíos marcados como pendientes.'}</span>
    ${n && n.error ? `<span><b>Error:</b> ${esc(n.error)}</span>` : ''}
    <span>Los conteos de la página son locales; por sí solos no prueban el guardado remoto.</span>`;
}
