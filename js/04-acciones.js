...
function googleSirve() { return false; } // OAuth no está implementado en esta versión.
...
function registrar(datos) {
  ...
  if (!datos.express) { const e = validarPass(datos.pass || ''); if (e) return { err: e }; }
  ...
  const u = {
    ...
    estado: 'activo', creado: new Date().toISOString(), verificado: false,
...
function abrirPublicar(art = null) {
  ...
  $('#formPublicar').reset();
  $('#minisPub').innerHTML = '';
  $('#zonaFotos').textContent = '📷 Sube al menos 1 foto (obligatorio) — hasta 6';
...
function pintarMinis() {
  ...
  $('#zonaFotos').textContent = App.fotos.length
    ...
    : '📷 Sube al menos 1 foto (obligatorio) — hasta 6';
}
function guardarPublicacion() {
  ...
  if (!App.fotos.length) return avisar('Debes subir al menos 1 foto del artículo', 'err');
...
function pintarChat() {
  ...
  const sinLeer = BD.mensajes.filter(m => m.chat === c.id && m.de !== u.id && !m.leido);
  sinLeer.forEach(m => m.leido = true);
  if (sinLeer.length) guardar();
