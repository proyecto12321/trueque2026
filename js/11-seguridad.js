...
/* Cuando alguien tampoco tiene el código de respaldo: deja su correo para
   que el administrador lo revise a mano y se lo mande (por WhatsApp, correo, etc). */
function solicitarCodigoAdmin(email, nota) {
  email = String(email || '').trim().toLowerCase();
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return { err: 'Escribe un correo válido.' };

  BD.solicitudesClave = BD.solicitudesClave || [];
  const yaPendiente = BD.solicitudesClave.some(s => s.email === email && !s.atendida);
  if (yaPendiente) return { err: 'Ya tienes una solicitud enviada. Espera a que el administrador la revise.' };

  const u = BD.usuarios.find(x => x.email.toLowerCase() === email);
  const sol = {
    id: nuevoId(), email, nota: String(nota || '').trim(),
    usuario: u ? u.id : null, nombre: u ? u.nombre : null,
    creado: new Date().toISOString(), atendida: false,
  };
  BD.solicitudesClave.unshift(sol);

  BD.usuarios.filter(x => x.rol === 'admin').forEach(a =>
    notificar(a.id, '🔑 Piden su código de respaldo',
      `${u ? u.nombre : email} olvidó su código de respaldo y pide ayuda para recuperarlo.`));

  anotarEvento('solicitud_codigo', `${email} pidió su código de respaldo al administrador`, u ? u.id : null);
  guardar();
  return { ok: true };
}

/* El administrador marca la solicitud como atendida (ya le mandó el código) */
function atenderSolicitudCodigo(id) {
  const s = (BD.solicitudesClave || []).find(x => x.id == id);
  if (!s) return;
  s.atendida = true;
  s.atendidaEn = new Date().toISOString();
  guardar();
}
...
function verSeguridad() {
  ...
  $('#seguridadCuerpo').innerHTML = `
    ...
    <p class="sub">${malos ? `Hay ${malos} punto(s) por mejorar.` : 'Comprobaciones locales completadas; no verifican los permisos del servidor.'}</p>
    <div class="diag">
    ...
    <div class="diag-arreglo">
      <b>Acceso a la base de datos</b>
      <p>Esta versión usa Supabase para los datos, pero las cuentas y los roles
      todavía se comprueban en el navegador. No equivale a Supabase Auth.</p>
      <p>Las políticas públicas de la tabla compartida permiten acceder a su contenido,
      incluidas conversaciones y datos de cuentas. La clave pública no limita ese acceso.</p>
      <p>Para protegerlo hace falta integrar Supabase Auth y políticas por propietario
      y participantes. Activar un proveedor en el panel no modifica este código.</p>
      <p>Publica la web mediante HTTPS. No copies reglas de otros servicios.</p>
    </div>
